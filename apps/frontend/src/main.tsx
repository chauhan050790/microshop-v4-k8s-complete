...
async function order(id:string){if(!token)return alert("Login first");const r=await fetch(API+"/orders",{method:"POST",headers:{"content-type":"application/json",authorization:"Bearer "+token},body:JSON.stringify({userId:"current-user",items:[{productId:id,quantity:1}]})});alert(r.ok?"Order created":"Order failed")}
