...
app.use((err:any,_req:any,res:any,_next:any)=>{console.error("user-service error",err);res.status(500).json({error:"internal server error"});});
export function startServer(){const server=app.listen(port,"0.0.0.0",()=>console.log(`user-service listening ${port}`));return server;}
